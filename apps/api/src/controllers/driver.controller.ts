...
export class DriverController {
    ...
    async acceptRequest(req: Request, res: Response) {
        ...
        try {
          ...
          console.error(error);
          return res.status(500).json({ message: "An error occurred while accepting the request." });
        }
      }
      ...
      async completeOrder(req: Request, res: Response) {
        ...
        try {
          ...
          if (updatedDriverOrder.count === 0) {
            return res.status(404).json({ message: "Order not found or not accepted yet." });
          }
